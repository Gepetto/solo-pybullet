...
def c(q, qdot, dt):
    ...
    torques = PD(
        qa_ref, qa_dot_ref, qa, qa_dot, dt, Kp, Kd, torque_sat, torques_ref
    )
...
def c_walking_IK(q, qdot, dt, solo, t_simu):
    ...
    fJ_FL3 = pin.frameJacobian(solo.model, solo.data, q_ref, ID_FL)[
        :3, -8:
    ...
    nu = np.concatenate((err_FL, err_FR, err_HL, err_HR))
    ...
    q_dot_ref = np.concatenate((np.zeros(6), qa_dot_ref))
    ...
    Kd = 0.2
    torque_sat = 3  # torque saturation in N.m
    torques_ref = np.zeros(8)  # feedforward torques
...
def c_walking_ID(q, qdot, dt, solo, t_simu):
    ...
    qu_dot = qdot[
        :6
    ]  # [v_x, v_y, v_z] linear velocity of the base and [w_x, w_y, w_z] angular velocity of the base along x, y, z axes of the world
    qa_dot = qdot[6:]  # angular velocity of the 8 motors
